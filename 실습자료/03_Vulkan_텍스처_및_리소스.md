# Vulkan 학습 자료집: 03. 텍스처 및 리소스 (심층 보강판)

이 문서는 `수정_03Vulkan (2).pdf` 파일의 내용을 바탕으로, 3D 객체에 생동감을 불어넣는 **텍스처 매핑(Texture Mapping)**과 3D 공간의 깊이를 올바르게 처리하는 **뎁스 버퍼링(Depth Buffering)**에 대해 상세한 비유와 심층 주석을 추가한 자료입니다.
...
- **`vkCmdPipelineBarrier`: GPU의 교통정리**
    > **심층 주석**: GPU는 명령을 받으면 순서대로 실행하지 않고, 가능한 한 병렬로 동시에 처리하여 효율을 극대화합니다. 하지만 어떤 작업들은 반드시 순서가 지켜져야 합니다. 예를 들어, '이미지에 데이터를 복사하는 작업(Transfer)'이 끝나기 전에 '셰이더가 그 이미지를 읽는 작업(Shader Read)'이 시작되면, 덜 복사된 깨진 이미지를 읽게 됩니다.
    > `vkCmdPipelineBarrier`는 바로 이럴 때 사용하는 '교통정리' 신호입니다.
    > - `srcStageMask`: "이전 단계들 중에서, **전송(Transfer) 단계**가 끝날 때까지는..."
    > - `dstStageMask`: "...다음 단계들 중에서, **프래그먼트 셰이더(Fragment Shader) 단계**는 시작하면 안 된다!"
    > - `srcAccessMask`: "특히, **쓰기(Write) 작업**이 완료되어야 하고,"
    > - `dstAccessMask`: "...이후 **읽기(Read) 작업**이 안전하게 접근할 수 있다."
    > 이렇게 장벽을 세워주면, GPU는 이 장벽 앞의 모든 쓰기 작업이 완료되었음을 보장한 뒤에야 다음 단계의 읽기 작업을 시작합니다. 이미지 레이아웃 전환은 이 파이프라인 장벽을 사용하여 특정 리소스에 대한 접근 방식이 변경되었음을 GPU에게 공식적으로 알려주는 행위입니다.
...
- **비유**: 텍스처라는 그림을 **'돋보기'**로 들여다볼 때의 규칙서입니다.
    ...
    > **심층 주석: 필터링과 성능의 관계**
    > `magFilter`와 `minFilter`는 텍스처 품질에 직접적인 영향을 줍니다. `VK_FILTER_LINEAR` (선형 보간)는 주변 텍셀들의 색상을 섞어 부드러운 결과를 만들지만, `VK_FILTER_NEAREST` (최단거리)는 가장 가까운 텍셀 하나만 사용하므로 계산이 더 빠르지만 계단 현상(픽셀화)이 보일 수 있습니다.
    > `anisotropyEnable` (비등방성 필터링)은 텍스처를 비스듬한 각도에서 볼 때 흐릿하게 보이는 현상을 크게 개선해주는 고급 필터링 기법입니다. 품질 향상 효과가 매우 크지만, 그만큼 GPU 연산 비용이 더 비쌉니다. 고품질 렌더링을 위해서는 거의 필수로 사용됩니다.
...
1.  **뎁스용 `VkImage`와 `VkImageView` 생성**: 컬러 이미지와 별개로, 깊이 정보를 저장할 전용 이미지와 뷰를 만듭니다.
    > **심층 주석: 뎁스 포맷의 의미 (`VK_FORMAT_D32_SFLOAT`)**
    > `D32_SFLOAT`는 깊이 값을 32비트 부동소수점(float)으로 저장하겠다는 의미입니다. 이는 매우 높은 정밀도를 제공하여, 물체들이 아주 가깝게 붙어있을 때 발생하는 Z-fighting(깊이 값이 비슷해서 어느 것이 앞인지 판단하지 못해 표면이 깜빡이는 현상)을 줄여줍니다. `D24_UNORM_S8_UINT` 같은 포맷은 깊이에 24비트, 스텐실(Stencil)에 8비트를 사용하는 등, 용도에 따라 다양한 포맷 중 적절한 것을 선택해야 합니다.
