...
public:
    ...
    {
        m_programID = LoadSplineShaders("Spline_VertexShader.txt", "Spline_FragmentShader.txt", "Spline_GeometryShader.txt");
    ...
    glm::vec3 getPointOnSpline(float t)
    {
        ...
        return 0.5f * ( (2.0f * p1) +
                        (-p0 + p2) * local_t +
                        (2.0f * p0 - 5.0f * p1 + 4.0f * p2 - p3) * lt2 +
                        (-p0 + 3.0f * p1 - 3.0f * p2 + p3) * lt3 );
    }
    ...
    void Draw(const glm::mat4& view, const glm::mat4& projection)
    {
        ...
        GLuint projID = glGetUniformLocation(m_programID, "projection");
        GLuint viewID = glGetUniformLocation(m_programID, "view");
...
private:
    ...
    // Shader Loader
    GLuint LoadSplineShaders(const char* vertex_file_path, const char* fragment_file_path, const char* geometry_file_path)
    {
